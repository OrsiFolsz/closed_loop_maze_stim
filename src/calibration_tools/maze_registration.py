...
def get_pixel_coords_from_image(image, label):
    ...
    # Invert the y-coordinate if needed (so 0 is at top left corner)
    image_height = image.shape[0]
    adjusted_coords = [(x, image_height - y) for (x, y) in pixel_coords]
...
def save_tower_alignment_coords(raw_video_path, tower_label2pixel_coords):
    ...
    with open(os.path.join(raw_video_path, "tower_center_pixel_coords.tsv"), "w") as outfile:
